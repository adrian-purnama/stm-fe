...
import { Plus, Search, Edit, Trash2, Eye, Loader2, Settings, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ChassisTypeTab = () => {
  ...
  const loadChassisTypes = useCallback(async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to load chassis types');
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    {chassisTypes.length > 0 && pagination.pages > 1 && (
