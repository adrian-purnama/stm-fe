...
import { Plus, Search, Edit, Trash2, Eye, Loader2, Truck, Settings } from 'lucide-react';
...
const TruckEntryTab = () => {
  ...
  const [chassisTypesLoading, setChassisTypesLoading] = useState(false);
  const [chassisTypesSearchTerm, setChassisTypesSearchTerm] = useState('');
  const [showChassisModal, setShowChassisModal] = useState(false);
  const [showChassisEditModal, setShowChassisEditModal] = useState(false);
  const [showChassisViewModal, setShowChassisViewModal] = useState(false);
  const [selectedChassisType, setSelectedChassisType] = useState(null);
  const [chassisFormData, setChassisFormData] = useState({
    name: '',
    shortName: ''
  });
  ...
  const updateChassisType = async () => {
    try {
      ...
      await ApiHelper.put(`/api/chassis-types/${selectedChassisType._id}`, chassisFormData);
      toast.success('Chassis type updated successfully');
      setShowChassisEditModal(false);
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="mb-4">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setBodyTypesSearchTerm(e.target.value)}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Manage chassis types</p>
          </div>
          <button
        ...
        <div className="mb-4">
          <div className="relative">
            ...
            <input
              ...
              value={chassisTypesSearchTerm}
              onChange={(e) => setChassisTypesSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        </div>
                ...
                <div key={chassisType._id} className="p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleChassisEdit(chassisType)}
                        className="inline-flex items-center px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors"
                      >
                        <Edit className="h-4 w-4 mr-1" />
                        Edit
      ...
      {/* Chassis Type Create Modal */}
      <BaseModal
        isOpen={showChassisModal}
        onClose={() => {
          setShowChassisModal(false);
          setChassisFormData({ name: '' });
        }}
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              value={chassisFormData.name}
              onChange={(e) => setChassisFormData({ ...chassisFormData, name: e.target.value })}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          ...
          <div>
            ...
            <input
              type="text"
              value={chassisFormData.shortName}
              onChange={(e) => setChassisFormData({ ...chassisFormData, shortName: e.target.value.toUpperCase() })}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent uppercase"
              placeholder="e.g., STD"
              maxLength={20}
            />
      ...
      {/* Chassis Type Edit Modal */}
      <BaseModal
        isOpen={showChassisEditModal}
        onClose={() => {
          setShowChassisEditModal(false);
          setSelectedChassisType(null);
          setChassisFormData({ name: '' });
      ...
      {/* Chassis Type View Modal */}
      <BaseModal
        isOpen={showChassisViewModal}
        onClose={() => {
          setShowChassisViewModal(false);
          setSelectedChassisType(null);
        }}
